...
#define TIEMPO_CIERRE_MAXIMO        (60 * 1000000)  // 30 seg: Máximo barreras cerradas
...
void app_main() {
    ...
    // gpio_set_level(PIN_BARRERA_ABRIR,1);
    // gpio_set_level(PIN_BARRERA_CERRAR,1);

    ESP_LOGI(TAG, "Sistema bidireccional iniciado correctamente");
    mostrar_estado_sistema();
    // bool state_out = false;
    // for (size_t i = 0; i < 10; i++){
    //     ESP_LOGI(TAG, "TEST NUM: %d",i);
    //     enviar_pulso_barrera(state_out);
