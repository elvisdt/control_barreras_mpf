...
#define TIEMPO_ENTRADA_TIMEOUT      (5 * 1000000)   // 5 seg: SA1→SA2 o SB2→SB1
#define TIEMPO_CIERRE_MAXIMO        (30 * 1000000)  // 30 seg: Máximo barreras cerradas
...
void enviar_pulso_barrera(bool abrir) {
    ...
    ESP_LOGI(TAG, ">>> %s BARRERAS", abrir ? "ABRIENDO" : "CERRANDO");
}
...
void callback_optimizacion(void* arg) {
    ESP_LOGI(TAG, "OPTIMIZACION: No hay nuevos vehiculos - ABRIENDO");
    abrir_barreras_y_resetear();
}
...
void procesar_cambio_sensor(gpio_num_t pin, bool estado) {
    ...
    ESP_LOGI(TAG, "Estado sensores: SA1=%d SA2=%d SB1=%d SB2=%d", 
...
void procesar_paso_peatonal(gpio_num_t pin, bool estado) {
    ...
    if (pin == PIN_SENSOR_SA1) {
        ESP_LOGI(TAG, "DETECCION A->B: Vehiculo aproximandose desde A");
        iniciar_procesamiento_sentido(GLOBAL_PROCESANDO_A_B);
        
    } else if (pin == PIN_SENSOR_SB2) {
        ESP_LOGI(TAG, "DETECCION B→A: Vehiculo aproximandose desde B");
...
void mostrar_estado_sistema() {
    ...
    ESP_LOGI(TAG, "Global Status: %s", obtener_nombre_estado_global(g_sistema.estado_global));
...
void init_timers() {
    ...
    timer_args.callback = &callback_entrada_timeout;
    timer_args.name = "entrada_timeout";
    esp_timer_create(&timer_args, &timer_entrada_timeout);
    
    // Timer cierre máximo
    timer_args.callback = &callback_cierre_maximo;
    timer_args.name = "cierre_maximo";
    esp_timer_create(&timer_args, &timer_cierre_maximo);
    
    // Timer optimización
    timer_args.callback = &callback_optimizacion;
    timer_args.name = "optimizacion";
    esp_timer_create(&timer_args, &timer_optimizacion);
}
